...
energy_derivs <- lapply(energy_mods, deriv_wrapper, seed_seed = NULL, ndraws = 500)
energy_deriv_summaries <- lapply(energy_derivs, derivs_summary)
energy_sign_summaries <- lapply(energy_derivs, sign_summary)

efits <- bind_rows(energy_fits)

ederivs <- bind_rows(energy_derivs)

ederivs_summaries <- bind_rows(energy_deriv_summaries)
...
efitplot <- ggplot(efits, aes(year, dependent, color = identifier)) +
  geom_point() +
  theme_bw() +
  geom_line(aes(year, fitted_value, color = identifier)) +
  facet_wrap(vars(identifier), scales = "free")
efitplot
ederivplot <- ggplot(ederivs, aes(year, derivative, group = seed, color = identifier)) +
  geom_line(alpha = .05) +
  geom_line(aes(year, mean)) +
  geom_line(aes(year, upper)) +
  geom_line(aes(year, lower)) +
  facet_wrap(vars(identifier), scales = "free")
#ederivplot
ggplot(ederivs_summaries, aes(identifier, net_percent_of_start, color = identifier)) +
  geom_boxplot() +
  theme_bw()+
  geom_hline(yintercept = 0)

ggplot(ederivs_summaries, aes(identifier, abs_v_net_change, color = identifier)) +
  geom_boxplot() +
  theme_bw()+
  geom_hline(yintercept = 0)

ederivs_means <- ederivs_summaries %>%
  group_by(identifier) %>%
  summarize(mean_net_change = mean(net_change),
            mean_net_percent_change = mean(net_percent_of_start),
            mean_abs_percent_change = mean(abs_percent_of_start),
...
emean_plot <- ggplot(ederivs_means, aes(mean_net_percent_change, mean_abs_percent_change, color = identifier)) +
  geom_point(size = 2) +
  geom_errorbar(aes(x = mean_net_percent_change, ymin = abs_percent_2p5, ymax = abs_percent_97p5)) +
   geom_errorbarh(aes(y = mean_abs_percent_change, xmin = net_percent_2p5, xmax = net_percent_97p5)) +
  theme_bw() +
...
```{r energy v abund, fig.dim = c(12,4)}

gridExtra::grid.arrange(grobs = list(fitplot, efitplot), nrow = 1)
#gridExtra::grid.arrange(grobs = list(derivplot, ederivplot), nrow = 1)
gridExtra::grid.arrange(grobs = list(mean_plot, emean_plot), nrow = 1)
