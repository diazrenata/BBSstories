...
joint_derivs <- joint_derivs_long %>%
  ...
  mutate(rnaught_mean = mean(rnaught),
         ...
         irat_mean = mean(irat),
         irat_upper = quantile(irat, probs = .975),
         irat_lower = quantile(irat, probs = .025)) %>%
  ungroup() %>%
  select(year, upper, lower, mean, currency, rnaught_mean, rnaught_upper, rnaught_lower,  irat_mean, irat_upper, irat_lower) %>%
  distinct()
...
ggplot(filter(joint_derivs, currency != "energy"), aes(year, irat_mean, color = currency, fill = currency)) +
  geom_line(aes(year, irat_mean)) +
  geom_ribbon(aes(year, ymin = irat_lower, ymax = irat_upper),  alpha = .25) +
  ggtitle("Instantaneous ratio for energy and abundance") +
  theme_bw() +
  scale_color_viridis_d() +
  scale_fill_viridis_d() +
  geom_hline(yintercept = 0)
...
ggplot(filter(joint_derivs), aes(year, irat_mean, color = currency, fill = currency)) +
  geom_line(aes(year, irat_mean)) +
  geom_ribbon(aes(year, ymin = irat_lower, ymax = irat_upper),  alpha = .25) +
  ggtitle("irat for energy and abundance") +
  theme_bw() +
  scale_color_viridis_d() +
  scale_fill_viridis_d() +
  geom_hline(yintercept = 0)
...
joint_derivs <- joint_derivs_long %>%
  ...
  mutate(rnaught_mean = mean(rnaught),
         ...
         irat_lower = quantile(irat, probs = .025)) %>%
  ungroup() %>%
  select(year, upper, lower, mean, currency, rnaught_mean, rnaught_upper, rnaught_lower,  irat_mean, irat_upper, irat_lower, identifier) %>%
  distinct()
...
ggplot(filter(all_derivs, currency != "energy"), aes(year, irat_mean, color = currency, fill = currency)) +
  geom_line(aes(year, irat_mean)) +
  geom_ribbon(aes(year, ymin = irat_lower, ymax = irat_upper),  alpha = .25) +
  ggtitle("Instant ratio for energy and abundance") +
  theme_bw() +
  scale_color_viridis_d() +
  scale_fill_viridis_d() +
  geom_hline(yintercept = 1) +
...
irat_summary <- all_derivs %>%
  group_by(currency, identifier) %>%
  summarize(mean_derivative = mean(mean),
            mean_irat = mean(irat_mean),
            irat_dev = mean(abs(irat_mean)))

irat_summary_wide <- irat_summary %>%
  select(currency, identifier, irat_dev) %>%
  tidyr::pivot_wider(identifier, names_from = currency, values_from = irat_dev, names_prefix = "irat_")

ggplot(irat_summary, aes(currency, irat_dev, color = currency)) +
  geom_point() +
  scale_color_viridis_d() +
  facet_wrap(vars(identifier))

ggplot(irat_summary_wide, aes(irat_abundance, irat_scaled_energy, color = identifier)) +
  geom_label(aes(label = identifier)) +
  scale_color_viridis_d() +
  theme_bw()

irat_summary_wide <- left_join(irat_summary_wide, all_summary_e_v_n)

ggplot(irat_summary_wide, aes(irat_abundance, abundance, color = identifier)) +
  geom_boxplot()+
  geom_label(aes(y = 1.3, label = identifier)) +
  scale_color_viridis_d() +
  theme_bw()

ggplot(irat_summary_wide, aes(irat_scaled_energy, scaled_energy, color = identifier)) +
  geom_boxplot()+
  geom_label(aes(y = 1.3, label = identifier)) +
  scale_color_viridis_d() +
  theme_bw()
