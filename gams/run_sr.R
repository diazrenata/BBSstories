...
write.csv(re_e_dfs, here::here("gams", "results", "ts_w_rescaled_e.csv"), row.names = F)
joint_samples <- bind_rows(joint_samples) %>%
  mutate(k = k_to_use)

write.csv(joint_samples, here::here("gams", "results", paste0("joint_samples_", k_to_use, ".csv")), row.names = F)
...
write.csv(both_change, here::here("gams", "results", paste0("both_change_", k_to_use, ".csv")), row.names = F)

write.csv(both_summary, here::here("gams", "results", paste0("both_summary_", k_to_use, ".csv")), row.names = F)
...
write.csv(both_ichange_summary, here::here("gams", "results", paste0("both_ichange_summary_", k_to_use, ".csv")), row.names = F)

write.csv(both_ichange_ts_summary, here::here("gams", "results", paste0("both_ichange_ts_summary_", k_to_use, ".csv")), row.names = F)
