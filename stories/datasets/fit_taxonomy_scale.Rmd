...
ggplot(filter(mod_summary, currency == "abundance"), aes(x = monotonic, group = cap_change, fill = cap_change)) +
  geom_bar(position = "dodge") + 
  theme_bw() +
  ggtitle("Abundance")


ggplot(filter(mod_summary, currency == "abundance"), aes(x = monotonic, group = lm_change, fill = lm_change)) +
  geom_bar(position = "dodge") + 
  theme_bw() +
  ggtitle("Abundance")


ggplot(filter(mod_summary, currency == "energy"), aes(x = monotonic, group = cap_change, fill = cap_change)) +
  geom_bar(position = "dodge") + 
  theme_bw() +
  ggtitle("Energy")


ggplot(filter(mod_summary, currency == "energy"), aes(x = monotonic, group = lm_change, fill = lm_change)) +
  geom_bar(position = "dodge") + 
  theme_bw() +
  ggtitle("Energy")


filter(mod_summary, currency == "abundance", monotonic) %>%
  summarize(n_lm_increase = sum(lm_increase),
            n_cap_increase = sum(cap_increase),
            n_lm_decrease = sum(lm_decrease),
            n_cap_decrease = sum(cap_decrease),
            n_lm_change = sum(lm_change),
            n_cap_change = sum(cap_change))
...
filter(mod_summary, currency == "energy", monotonic) %>%
  summarize(n_lm_increase = sum(lm_increase),
            n_cap_increase = sum(cap_increase),
            n_lm_decrease = sum(lm_decrease),
            n_cap_decrease = sum(cap_decrease),
...
mod_summary <- mod_summary %>%
  mutate(monotonic_word = ifelse(monotonic, "monotonic", "turn"))

ggplot(filter(mod_summary, currency == "abundance"), aes(x = cap_ratio)) +
  geom_histogram() +
  theme_bw() +
  facet_wrap(vars(cap_change, monotonic_word), scales = "free") +
  ggtitle("Abundance")

ggplot(filter(mod_summary, currency == "abundance"), aes(x = lm_ratio)) + 
 geom_histogram() +
  theme_bw() +
  facet_wrap(vars(lm_change, monotonic_word), scales = "free") +
    ggtitle("Abundance")


ggplot(filter(mod_summary, currency == "energy"), aes(x = cap_ratio)) +
  geom_histogram() +
  theme_bw() +
  facet_wrap(vars(cap_change, monotonic_word), scales = "free") +
  ggtitle("Energy")

ggplot(filter(mod_summary, currency == "energy"), aes(x = lm_ratio)) + 
 geom_histogram() +
  theme_bw() +
  facet_wrap(vars(lm_change, monotonic_word), scales = "free") +
    ggtitle("Energy")
...
ggplot(filter(mod_summary, currency == "abundance"), aes(x = monotonic, y = cv, color = cap_change)) + 
 geom_boxplot() +
  theme_bw() +
  ggtitle("Abundance")


ggplot(filter(mod_summary, currency == "energy"), aes(x = monotonic, y = cv, color = cap_change)) + 
 geom_boxplot() +
  theme_bw() +
  ggtitle("Energy")
```
...
ggplot(mod_summary, aes(cap_ratio_abundance, cap_ratio_energy)) +
  geom_point() +
  theme_bw() +
  geom_abline(slope = 1, intercept= 0)


ggplot(mod_summary, aes(lm_ratio_abundance, lm_ratio_energy)) +
  geom_point() +
  theme_bw() +
  geom_abline(slope = 1, intercept= 0)
